...
function buildNextItem() {
  ...
  if (toBuild[0].getAttribute('buildscript')) {
    var attr = toBuild[0].getAttribute('buildscript');
    var method, params;
    if (attr.indexOf(' ')<0) {
      method = attr;
    } else {
      method = attr.substr(attr, attr.indexOf(' '));
      params = attr.substr(attr.indexOf(' ')+1);
    }
    window[method](toBuild[0], params);
  }
...
function makeBuildLists() {
	for ( var i = curSlide, slide; slide = slideEls[i]; i++) {
        ...
        if (!item.getAttribute('buildscript')) {
          item.classList.add('build-default-transition');
        }
			}
		}
	}
